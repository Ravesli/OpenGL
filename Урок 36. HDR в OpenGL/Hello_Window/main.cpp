...
int main()
{
    ...
    // конфигурируем фреймбуфер с плавающей точкой
    // ------------------------------------
    unsigned int hdrFBO;
    glGenFramebuffers(1, &hdrFBO);
    // создание цветового буфера (с плавающей точкой)
    ...
    lightPositions.push_back(glm::vec3( 0.0f,  0.0f, 49.5f)); // подсветка
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // 1. рендеринг сцены в буфер с плавающей точкой
        // -----------------------------------------------
        glBindFramebuffer(GL_FRAMEBUFFER, hdrFBO);
            glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
        ...
        // 2. теперь рендерим цветовой буфер (с плавающей точкой) на 2D-прямоугольник и сужаем диапазон значений HDR-цветов к цветовому диапазону значений заданного по умолчанию фреймбуфера
        // --------------------------------------------------------------------------------------------------------------------------
        glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
        hdrShader.use();
        glActiveTexture(GL_TEXTURE0);
