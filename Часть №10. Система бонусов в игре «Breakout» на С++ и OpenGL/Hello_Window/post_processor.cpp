    ...
    : PostProcessingShader(shader), Texture(), Width(width), Height(height), Confuse(false), Chaos(false), Shake(false)
{
    ...
    // Также инициализируем FBO/текстуру для блиттирования мультисэмплированного цветового буфера; используется в шейдерных вычислениях (эффекты постпроцессинга)
