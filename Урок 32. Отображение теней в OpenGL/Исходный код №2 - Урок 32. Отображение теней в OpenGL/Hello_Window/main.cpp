...
unsigned int loadTexture(char const * path)
{
    ...
    if (data)
    {
        ...
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, format == GL_RGBA ? GL_CLAMP_TO_EDGE : GL_REPEAT); // для данного урока: испольщуйте GL_CLAMP_TO_EDGE для предотвращения возникновения полупрозрачных границ. Благодаря интерполяции берутся тексели из следующего повторения
