...
Collision CheckCollision(BallObject& one, GameObject& two) // столкновение вида AABB-Окружность
{
    ...
    if (glm::length(difference) < one.Radius) // именно "<", а не "<=", т.к. в противном случае в конце стадии разрешения столкновения (когда объекты касаются друг друга) произойдет повторное столкновение
